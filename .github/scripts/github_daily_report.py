...
def build_report(all_time, new_week, trending):
    ...
    def cn(full_name, text):
        native = native_cn.get(full_name)
        if native:
            return native
        translated = translations.get(text, text)
        if _has_chinese(translated):
            return translated
        if text and translated and translated != text:
    ...
    for i, repo in enumerate(trending[:15], 1):
        ...
        if isinstance(info, dict):
            ...
            d = (cn(repo, info.get("description") or ""))[:90]
        else:
            s, l, d = "?", "?", ""
    ...
    lines += ["", "---", "", f"*本报告由 GitHub Actions 每三天自动生成，数据截止 {TODAY}。*\n"]
    return "\n".join(lines)
            ...
            card = {
                ...
                "card": {
                    ...
                    "elements": [
                        ...
                        {"tag": "hr"},
                        {"tag": "note", "elements": [{"tag": "plain_text", "content": "每三天自动生成 · GitHub Actions"}]}
                    ]
                }
            }
